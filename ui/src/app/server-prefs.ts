...
function adoptPushWriter(writer: ServerUiPrefsWriter): void {
  ...
  const unscopedPending = pendingScope === "" ? pendingPrefs : null;
  clearConflictRedrain();
  pushEpoch += 1;
  pushWriter = writer;
  pushScope = scope;
  ...
  while (pendingPrefs) {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      if (result.ok) {
        ...
        clearConflictRedrain();
        // The serialized mutation refreshed while pending intent still
        // shadowed server values. Refresh once more after cleanup so a
        // concurrent server-side preference update is not hidden locally.
        afterCommit?.();
        if (pushWriter !== writer || pushEpoch !== epoch) {
          return;
        }
