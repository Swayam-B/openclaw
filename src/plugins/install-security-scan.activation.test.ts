...
import { scanBundleInstallSourceRuntime } from "./install-security-scan.runtime.js";
...
describe("install hook provider activation", () => {
  ...
  it("exposes only typed hooks during transient install scans", async () => {
    ...
    const result = await withInstallScanEnv(stateDir, () =>
      scanInstallPayload({ plugins: { load: { paths: [scanner.file] } } }),
    );
  ...
  it("does not treat the broad hook capability as an install scanner declaration", async () => {
    ...
    const result = await withInstallScanEnv(stateDir, () =>
      scanInstallPayload({ plugins: { load: { paths: [plugin.file] } } }),
    );
  ...
  it("discovers a hook provider from the target skill workspace", async () => {
    ...
    const result = await withInstallScanEnv(stateDir, () => scanSkillInstall(config, workspaceDir));
  ...
  it("does not reuse another workspace's persisted registry for an install scan", async () => {
    ...
    const result = await withInstallScanEnv(stateDir, async () => {
      ...
      return await scanSkillInstall(config, targetWorkspaceDir);
    });
