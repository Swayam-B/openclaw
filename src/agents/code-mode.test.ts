...
describe("Code Mode catalog and model-visible surface", () => {
  ...
  it("describes code-mode runtime constraints in the model-visible exec schema", () => {
    ...
    expect(execTool.description).toContain("`require`, and `import` are unavailable");
    expect(execTool.description).toContain("Prefer enabled direct methods on `tools`");
    expect(execTool.description).toContain("a trailing guest tool call is returned automatically");
    ...
    expect(execTool.description).toContain("return the raw value instead of guessing fields");
    const nodesGuidance =
    ...
    expect(parameters.properties?.code?.description).toContain(
      "Call enabled tools through `tools`",
    );
    expect(parameters.properties?.code?.description).toContain("return the final value");
