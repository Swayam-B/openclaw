...
`);
  ...
  it("adds optional Claw application provenance columns to existing state databases", () => {
    ...
    legacyDb.exec(`
      ...
      ALTER TABLE claw_workspace_files DROP COLUMN role;
    `);
    markStateDatabaseAsV5(legacyDb);
    legacyDb.close();
    ...
    const workspaceColumns = reopened.db
      .prepare("PRAGMA table_info(claw_workspace_files)")
      .all() as Array<{ name?: string }>;
    ...
    expect(workspaceColumns.map((column) => column.name)).toContain("role");
  });
