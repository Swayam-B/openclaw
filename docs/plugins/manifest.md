...
| `onHooks`          | No       | `Array<"before_install">`                            | Lifecycle hook dispatches that must load this plugin first. Declaring `before_install` opts an explicitly trusted plugin into install scanning.                                             |
...
- Install-hook planning loads every explicitly trusted plugin that declares `activation.onHooks: ["before_install"]` before `before_install` dispatch.
