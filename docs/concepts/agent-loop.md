...
| `before_install`                                        | After operator install policy runs, on staged skill/plugin install material. Explicitly trusted plugins declaring `activation.onHooks: ["before_install"]` are loaded before dispatch.                                                                                                      |
