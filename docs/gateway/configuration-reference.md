...
- `plugins.entries.<id>.llm.allowedModels`: optional allowlist of canonical `provider/model` targets for plugin LLM completions, including the resolved default. Use `"*"` only when you intentionally want to allow any model.
- `plugins.entries.<id>.llm.allowAuthProfileOverride`: explicitly trust this plugin to select a non-default auth profile for `api.runtime.llm.complete`.
