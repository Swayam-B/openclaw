...
import {
  ...
  type GeminiCliIsolatedAuthContext,
  projectGeminiCliSafeSettings,
...
type GeminiCliAuthHomeContext = GeminiCliIsolatedAuthContext & {
  agentDir?: string;
...
function applyGeminiCliIsolatedCompletionSettings(
  ...
  if (!modelId || modelId === "auto" || modelId.startsWith("auto-")) {
    throw new Error("Gemini isolated completion requires one concrete model id.");
  }
  const policy = {
    model: modelId,
    isLastResort: true,
  ...
  const modelConfigs = isRecord(base.modelConfigs) ? { ...base.modelConfigs } : {};
...
function applyGeminiCliToolAvailability(
  ...
  if (availability.openClaw.length === 0) {
    const security = isRecord(base.security) ? base.security : undefined;
    const auth = security && isRecord(security.auth) ? { ...security.auth } : undefined;
    return {
      ...projectGeminiCliSafeSettings(base),
      ...(auth ? { security: { auth } } : {}),
      tools: {
        core: [],
        allowed: [],
...
async function prepareGeminiCliOAuthHome(
  ...
  if (ctx.isolatedCompletionSystemPrompt !== undefined) {
    throw new Error(
      "Gemini CLI isolated completion does not support OAuth; Code Assist auth can inject administrator-required tools.",
    );
  }
...
async function prepareGeminiCliApiKeyHome(
  ...
  const isolatedTransportEnv = profileHome.isolatedSystemPromptPath
  ...
  return {
    env: {
      ...
      ...(profileHome.isolatedSystemPromptPath ? GEMINI_CLI_ISOLATED_ENV_BARRIERS : {}),
      ...isolatedTransportEnv,
      ...(profileHome.isolatedSystemPromptPath
        ? { GEMINI_SYSTEM_MD: profileHome.isolatedSystemPromptPath }
        : {}),
      ...(profileHome.isolatedSystemPromptPath ? { GEMINI_TELEMETRY_LOG_PROMPTS: "false" } : {}),
    ...
    clearEnv: [
      ...
      ...(profileHome.isolatedSystemPromptPath
        ...
        : []),
      ...(profileHome.isolatedSystemPromptPath
        ? Object.keys(GEMINI_CLI_ISOLATED_ENV_BARRIERS)
        : []),
      ...Object.keys(isolatedTransportEnv ?? {}),
    ],
    beforeExecution: async () => {
      await profileHome.beforeExecution();
      await Promise.all([
...
async function prepareGeminiCliRestrictedSystemSettings(
  ...
  const isolated = ctx.isolatedCompletionSystemPrompt !== undefined;
  const isolatedCompletionEnforced = assertGeminiCliLiteralIsolatedPrompt(ctx);
  const ambientAuth = isolated ? await resolveGeminiCliAmbientAuth(ctx) : undefined;
